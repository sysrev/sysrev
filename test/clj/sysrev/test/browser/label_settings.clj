...
(deftest-browser label-consensus-test
  ...
  (do (nav/log-in (:email test-user))
      ...
      (b/wait-until-loading-completes :pre-wait 50 :loop 3)
      (b/click "div.article-list-article" :delay 100)
      ;; check for conflict label in article component
      (b/exists? ".label.review-status.orange")
      ...
      (b/wait-until-loading-completes :pre-wait 50 :loop 3)
      (b/click "div.article-list-article" :delay 100)
      (b/text-is? ".button.change-labels" "Resolve Labels")
      (b/click ".button.change-labels")
      (b/click ".button.save-labels" :delay 100)
      ...
      (b/exists? ".ui.label.review-status.purple")
      (b/exists? ".ui.label.labels-status.purple"))
