(ns sysrev.views.panels.landing_pages.systematic_review
...
(defn IntroSegment []
  [:div.ui.segment.center.aligned.inverted {:style {:padding-top 50 :padding-bottom 40 :margin-top -13
   ...
   [:div.description.wrapper.open-sans {:style {:margin "auto" :max-width "500px" :margin-bottom 20}}
    [:h1.ui {:style {:margin-top 5 :font-size "48px"}} "Built for Systematic Review."]
    [:h2.ui {:style {:margin-top 5 :font-size "20px"}} "Search the literature."[:br]"Upload research documents."[:br]"Add friends and review."]
...
(defn FeaturedReviews []
  [:div.ui.segment.center.aligned {:style {:padding-top 60 :margin-top 0 :border 0 :border-radius 0 :margin-bottom 0}}
   [:div.description.wrapper.open-sans {:style {:margin "auto" :max-width "600px" :padding-bottom 0}}
    ...
    [:h2.ui [:a {:href "/search"} "Discover "] "and"
     [:a {:href "https://blog.sysrev.com/cloning-projects"} " clone"] " public systematic reviews."[:br]
   ...
   [:div.ui.attached.three.stackable.cards {:style {:max-width "1000px" :margin "auto"}}
    ...
    [:div.ui.raised.card
     ...
     [:div.content
      ...
      [:div.description "A systematic review of clinicaltrials.gov measuring drugs and dosing in ascorbate cancer trials."
       [:a {:href "https://scholar.google.com/scholar?cluster=16503083734790425316&hl=en&oi=scholarr"} " Vitamin C and Cancer: An Overview of Recent Clinical Trials"]]]
     [:div.extra.content "Channing Paller"[:br]" Johns Hopkins School of Medicine."[:br] "with collaborators at EMMES"
      ]]]
   ])
