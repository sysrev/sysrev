...
(defn save-file
  [^java.io.File file & {:keys [bucket-name] :or {bucket-name pdf-bucket}}]
  (let [sha-1-hash (-> file
...
(defn save-byte-array
  [byte-array & {:keys [bucket-name] :or {bucket-name pdf-bucket}}]
...
(defn get-file
  ...
  [file-key & {:keys [bucket-name] :or {bucket-name pdf-bucket}}]
