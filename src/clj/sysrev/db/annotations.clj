...
(defn update-annotation!
  ...
  (with-transaction
    ...
    (let [semantic-class (if (empty? semantic-class)
                           nil
                           semantic-class)
          current-semantic-class-id (-> (select :semantic-class-id)
                                        (from :annotation-semantic-class)
                                        (where [:= :annotation-id annotation-id])
  ...
  (with-project-cache
    ...
    (-> (select :an.created :aa.article-id :sc.definition :au.user-id)
        ...
        (join [:annotation-article :aa]
              ...
              [:annotation-web-user :au]
              [:= :au.annotation-id :an.id])
        (left-join [:annotation-semantic-class :asc]
                   [:= :an.id :asc.annotation-id]
...
(defn text-context-article-field-match
  ...
  (let [article (-> (select :primary-title :secondary-title :abstract)
                    (from :article)
                    (where [:= :article-id article-id])
                    do-query first)
