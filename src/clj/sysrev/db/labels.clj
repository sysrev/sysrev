...
(def boolean-definition-validations
  {:inclusion-values [[v/required :message "Inclusion values must be included"]
                      [boolean-or-nil? :message "Inclusion values must be boolean or nil"]]})

(def string-definition-validations
  {:multi? [[v/required :message "A setting for multiple responses must be made"]
            [boolean-or-nil? :message "Allow multiple values must be true, false or nil"]]
   :examples [[v/every string? :message "Examples must be strings"]]
   :max-length [[v/required :message "A max length must be provided"]
                [v/integer :message "Max length must be defined by an integer"]]
   :entity [[v/string :message "Entity must be defined by a string"]]})

(def categorical-definition-validations
  {:multi? [[v/required :message "A setting for multiple responses must be made"]
            [boolean-or-nil? :message "Allow multiple values must be true, false or nil"]]
   :all-values [[v/required :message "A category must have defined options"]
                [v/every string? :message "All options must be strings"]]
   :inclusion-values [[v/every string? :message "All inclusion values must be strings"]]})

(defn label-validations
  [value-type]
  {:value-type [[v/required :message "A label must have a type"]
                [v/string :message "Label type must be a string"]
                [(partial contains? (set valid-label-value-types)) :message (str "A label must of type " (str (str/join "," (rest valid-label-value-types)) " or " (first valid-label-value-types)))]]
   :project-id [[v/required :message "Project ID must not be blank"]
                [v/integer :message "Project ID must be an integer"]]
   ;; these are going to be generated by the client so shouldn't
   ;; be blank, checking anyway
   :name       [[v/required :message "Name must not be blank"]
                [v/string :message "Name must be a string"]]
   :question   [[v/required :message "Question can not be blank"]
                [v/string :message "Question must be a string"]]
   :short-label [[v/required :message "Label must have a name"]
                 [v/string :message "Label must be a string"]]
   :inclusion-values [[boolean-or-nil? :message "Inclusion must be true, false or nil"]]
   :required [[boolean-or-nil? :message "Required must be true, false or nil"]]
   :definition (condp = value-type
                 "boolean" boolean-definition-validations
                 "string" string-definition-validations
                 "categorical" categorical-definition-validations
                 {})})
