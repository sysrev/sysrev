...
(defn count-categorical-vals [project-id]
  (-> (select :l.label-id :l.short-label :l.value-type :o1.value :o1.count)
      ...
      (join [(-> (select :al.label-id :value :%count.*)
                 ...
                 (where [:and [:= :%jsonb_typeof.answer "array"]
                         (q/exists [:label :l2] {:l2.project-id project-id
                                                 :l2.label-id :al.label-id
                                                 :l2.value-type "categorical"}
...
(defn process-label-counts [project-id]
  (db/with-project-cache project-id [:charts-label-counts]
    (db/with-transaction
