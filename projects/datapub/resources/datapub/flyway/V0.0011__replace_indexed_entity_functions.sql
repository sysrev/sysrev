...
begin
  ...
  if exist then
    execute format('insert into %I (entity_id, indexed_data) values (%L, (select data from content_file where content_id=%L));', ie_table, new.id, new.content_id);
...
begin
  ...
  if not exist then
    ...
    execute format('insert into %I (entity_id, indexed_data)
                    select entity.id, content_file.data
                    from entity
                    join content_file on entity.content_id = content_file.content_id
